...
local function init()
    ...
    for _, file in ipairs(files) do 
        ...
        if success then 
            ui_text[code] = array.deep_copy(ui_text.default)
            array.merge(ui_text[code], loaded_lang)
            table.insert(language_list, code)
            local language_name            = loaded_lang.language_name
            local index                    = #language_list            
            language_lookup[code]          = index
            language_lookup[language_name] = index
...
local this  = {
    get_ui_text   = function(lang_code) return ui_text[lang_code]    or  ui_text.default end,
    get_code_list = function()          return (#language_list > 0)  and language_list   end,
    get_name_list = function()          return (#language_names > 0) and language_names  end,
    get_lookup    = function(code)      return language_lookup[code]                     end,
}
